...
public class CountryService {
	...
	public List<Country> getAllCountries() {
		...
		/*
		 * List countries = new ArrayList(countryIdMap.values()); // countryIdMap is
		 * hashmap when we use values() it will // be added into list return countries;
		 */
	}
	...
	public Country getCountryByName(String countryName) {
		 ...
		 * 
		 * 
		 * for (int i : countryIdMap.keySet()) { // keySet() -> will return all the key
		 * from the HashMap if
		 * (countryIdMap.get(i).getCountryName().equals(countryName)) { country =
		 * countryIdMap.get(i); // whole object -> whole one entry } } return country;
		 */
	}
	...
	public AddResponse deleteCountry(int id) {
		 ...
		 * res.setMsg("Country deleted..."); res.setId(id); return res; // here it will
		 * be automatically convert into json object from spring boot // framework (in
		 * others we need to import jackson api do manually)
		 */
	}
	...
	public int getMaxId() {
		...
		/*
		 * int max = 0; for (int id : countryIdMap.keySet()) { if (max <= id) { max =
		 * id; } }
		 * 
		 * return max + 1;
		 */
